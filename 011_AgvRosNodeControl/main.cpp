...
#define DEBUG_AMCL_POS_SUBSCRIBE
#define DEBUG_ROS
...
void ReceivedAmclPos(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msgAMCL);
...
void RosRunning(){
  ...
  //ros::NodeHandle nSub;
  //ros::Subscriber sub = nSub.subscribe("/amcl_pose", 5000, ReceivedAmclPos);
  ...
  while(n.ok()){
    ...
    vx = (vLeft_t + vRight_t)/2*(1);
    vth = (-vRight_t + vLeft_t)/0.4;
...
void ReceivedAmclPos(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msgAMCL){
  ...
  #ifdef DEBUG_AMCL_POS_SUBSCRIBE
    std::cout << "\nAMCL Pos received... " + std::to_string(poseAMCLx) + "-" + std::to_string(poseAMCLy) + "-" + std::to_string(poseAMCLa);
  #endif
}